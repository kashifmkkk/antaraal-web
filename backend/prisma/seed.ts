...
async function main() {
  ...
  const adminPasswordHash = await bcrypt.hash('admin@skyway1', 10);
  ...
  const vendorPasswordHash = await bcrypt.hash('password', 10);
  ...
  const buyerPasswordHash = await bcrypt.hash('buyerpass', 10);
  await prisma.user.upsert({
